...
export default function Home(){
 ...
 useEffect(()=>{const tick=()=>setTime(new Date().toLocaleString('en-US',{weekday:'short',month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}));tick();const id=setInterval(tick,30000);return()=>clearInterval(id)},[]);

 return <main className="desktop"><div className="wallpaper"/><header className="topbar"><button className="monogram" aria-label="Restore desktop" onClick={()=>{setOpen(['work','activity','contact']);setExpanded(null)}}><Command size={17}/></button><Menubar className="desktop-menu"><MenubarMenu><MenubarTrigger className="app-name">Luciano Pinilla</MenubarTrigger><MenubarContent><MenubarItem onClick={()=>show('contact')}>Get in touch</MenubarItem></MenubarContent></MenubarMenu><MenubarMenu><MenubarTrigger>Work</MenubarTrigger><MenubarContent><MenubarItem onClick={()=>show('work')}>Fomo Campus</MenubarItem><MenubarItem onClick={()=>show('activity')}>GitHub activity</MenubarItem></MenubarContent></MenubarMenu><MenubarMenu><MenubarTrigger>Window</MenubarTrigger><MenubarContent>{apps.map(a=><MenubarItem key={a.id} onClick={()=>show(a.id as AppId)}>{a.name}</MenubarItem>)}<MenubarItem onClick={()=>{setOpen([]);setExpanded(null)}}>Show desktop</MenubarItem></MenubarContent></MenubarMenu></Menubar><div className="menu-status"><time className="desktop-clock">{time}</time></div></header>
 <div className="desktop-heading"><h1>luciano<br/>pinilla</h1><div className="identity-links"><a href="https://x.com/foezart" target="_blank" rel="noopener noreferrer">@foezart ↗</a></div></div>
 <div className="workspace">
 <Window open={open} front={front} expanded={expanded} setFront={setFront} setOpen={setOpen} setExpanded={setExpanded} id="work"><div className="work-toolbar"><span><span className="tiny-folder">▰</span> My Work / Fomo Campus</span><span>01</span></div><a href="https://fomo-campus-wars-luchi.luciano589188.chatgpt.site/" target="_blank" rel="noopener noreferrer" className="project-image"><img src="/images/fomo-village.jpg" alt="The Greek village from the Fomo Campus website"/><span className="project-image-label">fomo<span>campus</span></span><span className="image-arrow"><ArrowUpRight size={24}/></span></a><div className="project-copy"><div className="eyebrow">FOMO CAMPUS</div><h2>Interning at Fomo.</h2><p>Handling campus outreach, onboarding fraternities into Greek Wars, and building brand awareness for Fomo.</p><div className="project-bottom"><span className="project-tags">Web design <b>·</b> Development</span><Out href="https://github.com/ouchip/fomo-campus">View source</Out></div></div></Window>
 <Window open={open} front={front} expanded={expanded} setFront={setFront} setOpen={setOpen} setExpanded={setExpanded} id="activity"><div className="activity-body"><div className="activity-heading"><div><h2>A little more green.</h2></div><GitBranch size={26}/></div><Activity/><Out className="activity-link" href="https://github.com/ouchip">github.com/ouchip</Out></div></Window>
 <Window open={open} front={front} expanded={expanded} setFront={setFront} setOpen={setOpen} setExpanded={setExpanded} id="contact"><div className="contact-body"><h2>Let’s talk.</h2><p>Reach out on X.</p><Out className="contact-link" href="https://x.com/foezart">@foezart</Out></div></Window>
